// ═══════════════════════════════════════════════════════════════
//  길드 정산 시스템 v11.8  (분배비중 · 연합 · 레이드 · 용어사전 · 게시판 · 3개국어)
//  시트 구성: [사용안내] [멤버DB] [참여자현황] [분배대기중] [잔액현황]
//            [지급기록] [연합] [레이드] [용어] [게시판] [작업기록] + [시즌1] [시즌2] ...
//            ← 이 순서로 항상 정렬됨
// ═══════════════════════════════════════════════════════════════
//  변경점 v11.7 → v11.8  (삭제는 마스터 전용 · 끝난 건은 아예 못 지운다)
//
//   - ★ 권한을 가르는 기준을 **"되돌릴 수 있는가"** 하나로 다시 세웠다.
//       고치는 것은 되돌릴 수 있고(기록이 남는다), 지우는 것은 되돌릴 수 없다.
//
//         관리자  → 등록된 것을 **고치는 것까지**
//         마스터  → 위 전부 + **지우는 것**
//
//   - ★ 연합 정산이 끝난 건은 **아무도 못 지운다** (아이템 분배완료와 같은 규칙).
//       혈맹운영비에 실제로 적립된 뒤라, 지우면 "그때 얼마가 들어왔다"는 사실이
//       사라진다. 잘못된 것은 [수정]으로 고친다 — 혈비를 차액만 조정하므로
//       결과는 같으면서 기록이 남는다.
//   - ★ `api_editAlliance` 의 `asMaster` 를 없앴다. 정산된 건도 **관리자**가
//       고친다 — 삭제라는 탈출구를 막았으므로, 고칠 길까지 막으면 잘못된 정산이
//       마스터를 기다리는 동안 그대로 남는다. 안전장치는 그대로다:
//       바뀔 혈비를 보여준 뒤 `confirm === true` 로 다시 불러야 실행된다.
//   - ★ 연합 삭제(미정산 건)는 **마스터 전용**이 됐다. Vercel 쪽에서도
//       /api/master/alliance 로 옮겨 경로만 봐도 등급이 드러나게 했다.
//
// ═══════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════
...
function _rebuildGuide(ss) {
  ...
  const L = [
    ...
    ['  (아직 정산 안 된 건을 지우면 적립될 것이 없으므로 잔액도 그대로입니다)', 'b'],
    ['· 연합 인원은 우리 멤버DB와 무관하므로 "몇 명"만 셉니다 — 누구인지는', 'b'],
    ['  판별하지 않습니다. 개인 잔액은 어느 단계에서도 바뀌지 않습니다', 'b'],
    ['· ⭐ 수정·삭제 권한 (v11.8):', 'b'],
    ['    관리자 → 고치는 것까지 (아이템명·서버·인원·사진, 정산된 건의 판매금액도)', 'b'],
    ['    마스터 → 위 전부 + 지우는 것 (아직 정산 안 된 건만)', 'b'],
    ['  ⭐ 정산이 끝난 건은 **아무도 지울 수 없습니다** — [수정]으로 고쳐주세요', 'warn'],
    ['     (혈비가 이미 적립된 뒤라, 지우면 얼마가 들어왔는지가 사라집니다.', 'warn'],
    ['      수정은 혈비를 차액만 조정하므로 결과는 같고 기록이 남습니다)', 'warn'],
    ['  정산된 건을 고치면 바뀔 ' + FUND_NAME + ' 를 먼저 보여주고 다시 확인합니다', 'warn'],
...
function api_editAlliance(group, item, entries, amount, email, confirm) {
  ...
  try {
    ...
    /*
     * ★ 정산된 건도 **관리자**가 고친다 (v11.8).
     *
     * v11.7 까지는 마스터 전용이었다. 그때는 잘못된 정산을 지워버리는 길이
     * 있었지만, v11.8 에서 **정산된 건의 삭제를 없앴다.** 지우는 길과 고치는 길을
     * 둘 다 막으면 잘못된 혈비가 마스터를 기다리는 동안 장부에 그대로 남는다.
     *
     * 여는 근거는 "안전해서" 가 아니라 **되돌릴 수 있어서** 다 — 이 수정은
     * 기록을 지우지 않고 혈비를 **차액만** 조정하므로, 잘못 고쳤으면 다시 고치면 된다.
     * 대신 아래 confirm 게이트가 그대로 남는다: 바뀔 혈비를 보여준 뒤에만 실행된다.
     */
...
function api_deleteAlliance(group, email) {
  ...
  const vals = sheet.getRange(2, 1, sheet.getLastRow() - 1, ALLIANCE_HEADERS.length).getValues();
  const hit = [];
  let item = '', credited = 0, done = false;
  vals.forEach(function (r, i) {
    const g = String(r[ALLY_COL.GROUP - 1]).trim() || ('r' + (i + 2));
    if (g !== group) return;
    const amt = Number(String(r[ALLY_COL.AMOUNT - 1]).replace(/,/g, '')) || 0;
    ...
    item = String(r[ALLY_COL.ITEM - 1]).trim();
    credited += Number(String(r[ALLY_COL.CREDITED - 1]).replace(/,/g, '')) || 0;
    // v10.2 이하 행에는 상태 칸이 없다 — 그때는 금액 유무로 본다 (조회와 같은 규칙)
    const st = String(r[ALLY_COL.STATUS - 1]).trim() || (amt > 0 ? ST_DONE : ST_WAIT);
    if (st === ST_DONE) done = true;
  });
  if (hit.length === 0) return _rc({ ok: false, msg: '기록을 찾을 수 없습니다.' }, 'e.noRecord');
  ...
  /*
   * ★ 정산이 끝난 건은 **지우지 않는다** (v11.8, 아이템 분배완료와 같은 규칙).
   *
   * 혈비는 이미 혈맹운영비 잔액에 실제로 적립됐다. 적립분을 되돌릴 수는 있어도
   * "그때 이 아이템이 얼마에 팔려 얼마가 들어왔다" 는 사실은 되돌릴 수 없는데,
   * 행을 지우면 그것까지 사라진다. 잘못된 것은 [수정]으로 고친다 — 혈비를
   * **차액만** 조정하므로 결과는 같으면서 기록이 남는다.
   *
   * ★ 판정은 **여기서** 한다. 화면에서 버튼만 감추면 라우트를 직접 부르는 길이
   *   그대로 열려 있다 (규칙 5-3).
   */
  if (done) {
    return _rc({
      ok: false, reason: 'done',
      msg: '이미 정산된 "' + item + '" 은(는) 삭제할 수 없습니다. [수정]으로 고쳐주세요.'
    }, 'e.allyNoDelete', { item: item });
  }
  ...
  return _rc({ ok: true, msg: '✅ 삭제했습니다 — ' + detail },
    'ally.delMulti', { item: item, credited: credited, fund: 0 });
}
...
function _apiRoute(action, req) {
  switch (action) {
    ...
    /*
     * 연합 항목 수정 (v11.3 → v11.8) — 미정산도 정산된 건도 **관리자 이상**이 고친다.
     * 정산된 건은 시트가 `confirm === true` 없이는 실행하지 않고, 바뀔 혈비를
     * 먼저 돌려준다. 그 판정은 **시트가** 하므로 라우트를 직접 불러도 뚫리지 않는다.
     */
    case 'editAlliance':
